    ...
    private AccessibilityNodeInfo getRootNodeInfo() {
        ...
        if (Build.VERSION.SDK_INT >= 16) {
            ...
            if (mAccessibilityService != null) {
                ...
                PrintUtils.buslog("nodeInfo: " + nodeInfo);
            }
...
//            nodeInfo = curEvent.getSource();
    ...
    public boolean scrollForward(AccessibilityNodeInfo nodeInfo) {
        if (nodeInfo != null && nodeInfo.isEnabled()) {
            // 获得点击View的类型
            PrintUtils.buslog("View类型：" + nodeInfo.getClassName());
            // 进行模拟滚动
    ...
    public boolean click(AccessibilityNodeInfo nodeInfo) {
        if (nodeInfo != null && nodeInfo.isEnabled()) {
            // 获得点击View的类型
            PrintUtils.buslog("View类型：" + nodeInfo.getClassName());
            // 进行模拟点击
    ...
    private boolean performClick(List<AccessibilityNodeInfo> nodeInfos) {
        if (nodeInfos != null && !nodeInfos.isEmpty()) {
            ...
            for (int i = 0; i < nodeInfos.size(); i++) {
                ...
                // 获得点击View的类型
                PrintUtils.log("View类型：" + node.getClassName());
                // 进行模拟点击
                if (node.isEnabled()) {
